...
int Enumerator::get_arity_of_term(int start_index, const string& tree){
    /*
    Determines the arity of the operator which can be placed at index
    ...
    start_index: int
        ...
        bracket within the `GOURGS` generated string tree
        
    tree: string
        The solution tree as generated by `GOURGS`
    
    Returns
    -------
    arity: int
        the arity of the operator at `start_index` in `tree`
    */
...
void Enumerator::decimal_to_base_m(int v, int m, vector<int>& e_base_arity)
{
    ...
    e_base_arity: vector<int>
        Empty vector in which the decimal number's base `m` representation
        is stored
        
    Returns
    -------
    None, edits e_base_arity    
    */
    if(v < 0)
    {
...
void Enumerator::base_m_to_decimal(const std::vector<int>& v, int m, 
                                   vector<int>& list_bits_deci)
{
...
void Enumerator::base_computations(int e, int m, 
                                   std::vector<int>& list_bits_deci)
{
...
void Enumerator::deinterleave(vector<int> num, int m, vector<int>& elemsLin)
{
    ...
    elemsLin: vector<int>&
        Reference to vector of integers where the deinterleaved values are to 
        be placed
        
    Returns
    -------
    None : elemsLin is edited in place
    */
                                                ...
                                                int index)
{
    ...
    repeat: int
        If `pools` is only one object, `repeat` specifies the number of times
